...
class DoctorProfileApi {
  ...
  async getProfile(userType, emailId) {
    ...
    const result = await this.makeRequest('/v1/doctor/profile', {
      method: 'POST',
    ...
    if (result.success) {
      ...
      if (result.status === 403) {
        return { success: false, error: 'Access denied. Only doctors can access this profile.' };
  ...
  async uploadProfilePicture(userType, emailId, imageFile) {
    ...
    const result = await this.makeRequest('/v1/doctor/profile/picture', {
      method: 'POST',
      body: formData,
      headers: {
...
const doctorProfileApi = new DoctorProfileApi();
