...
class PatientProfileApi {
  ...
  async getProfile(userType, emailId) {
    ...
    const result = await this.makeRequest('/v1/patient/profile', {
      method: 'POST',
    ...
    if (result.success) {
      ...
      if (result.status === 403) {
        return { success: false, error: 'Access denied. Only patients can access this profile.' };
  ...
  async uploadProfilePicture(userType, emailId, imageFile) {
    ...
    const result = await this.makeRequest('/v1/patient/profile/picture', {
      method: 'POST',
      body: formData,
      headers: {
...
const patientProfileApi = new PatientProfileApi();
